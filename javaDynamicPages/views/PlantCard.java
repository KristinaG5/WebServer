...
public class PlantCard extends DynamicWebPage
{
	MVMap<Integer, Plant> database = db.s.openMap("plant");
	private Plant plant = database.get(1);
	public int id = 0;

	public PlantCard(DatabaseInterface db,FileStoreInterface fs)
	{
		super(db,fs);
	}
	...
	{
		if(toProcess.path.equalsIgnoreCase("PlantCard.html"))
		{
			...
			stringToSendToWebBrowser += "\t\t\t<p class=\"animalintro\"><span>Fun Fact</span>: "+plant.getFunfact()+"</p>\n";
			stringToSendToWebBrowser += "\t\t\t<ul>\n";
			stringToSendToWebBrowser += "\t\t\t\t<li><span>Family</span>: "+plant.getFamily()+"</li>\n";
			stringToSendToWebBrowser += "\t\t\t\t<li><span>Location</span>: "+plant.getLocation()+"</li>\n";
			stringToSendToWebBrowser += "\t\t\t</ul>\n";
			stringToSendToWebBrowser += "\t\t\t<p>\n";
			stringToSendToWebBrowser += "\t\t\t<span>Bio</span>:\n";
			stringToSendToWebBrowser += "\t\t\t\t"+plant.getBio()+"\n";
			stringToSendToWebBrowser += "\t\t\t</p>\n";
			stringToSendToWebBrowser += "\t\t</div>\n";
			stringToSendToWebBrowser += "<a href=\"PlantEdit.html/"+id+"\"><input type=\"submit\" name=\"submit\" id=\"submit\" value=\"Edit\"\"></a>\t\n";
			stringToSendToWebBrowser += "\t</div>\n";
			stringToSendToWebBrowser += "\n";
			stringToSendToWebBrowser += "</div>\n";
			stringToSendToWebBrowser += "\n";
			...
			return true;
		}else if(toProcess.path.contains("PlantCard.html")) {
			try {
				MVMap<Integer, Plant> database = db.s.openMap("plant");
			...
			toProcess.r = new WebResponse( WebResponse.HTTP_OK, WebResponse.MIME_HTML,  "<html><head><meta http-equiv=\"refresh\" content=\"0; url=../PlantCard.html\" /></head></html>" );
			return true;
		}
		return false;
	}
