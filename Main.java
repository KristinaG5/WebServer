...
import views.Login;
import views.Signup;
import views.AnimalCard;
import views.PlantCard;
import views.SearchPage;
import views.ContactPage;
import views.Index;
import views.Logout;
import views.AddAnimal;
import views.AddPlant;
import views.LoginSubmit;
import views.EditAccountPage;
import views.AnimalEdit;
import views.PlantEdit;
...
public class Main 
{
	...
	{		
		...
		registerShutdownHook(databaseInterface);		System.out.println("Project:csc0");

		Login iLogin = new Login(databaseInterface,fileStoreInterface);

		Signup iSignup = new Signup(databaseInterface,fileStoreInterface);

		AnimalCard iAnimalCard = new AnimalCard(databaseInterface,fileStoreInterface);

		PlantCard iPlantCard = new PlantCard(databaseInterface,fileStoreInterface);

		SearchPage iSearchPage = new SearchPage(databaseInterface,fileStoreInterface);

		ContactPage iContactPage = new ContactPage(databaseInterface,fileStoreInterface);

		Index iIndex = new Index(databaseInterface,fileStoreInterface);

		Logout iLogout = new Logout(databaseInterface,fileStoreInterface);

		AddAnimal iAddAnimal = new AddAnimal(databaseInterface,fileStoreInterface);

		AddPlant iAddPlant = new AddPlant(databaseInterface,fileStoreInterface);

		LoginSubmit iLoginSubmit = new LoginSubmit(databaseInterface,fileStoreInterface);

		EditAccountPage iEditAccountPage = new EditAccountPage(databaseInterface,fileStoreInterface);

		AnimalEdit iAnimalEdit = new AnimalEdit(databaseInterface,fileStoreInterface);

		PlantEdit iPlantEdit = new PlantEdit(databaseInterface,fileStoreInterface);
			...
			for (int i = 0; i < requestToProcess.size(); i++) 
			{
		        ...
		        //the example dynamic page will process the request if the index.html file is not present
		        	if(toProcess.path.length()==0)
		        		toProcess.path = "index.html";		if(iLogin.process(toProcess))
		{
		}
		else
		if(iSignup.process(toProcess))
		{
		}
		else
		if(iAnimalCard.process(toProcess))
		{
		}
		else
		if(iPlantCard.process(toProcess))
		{
		}
		else
		if(iSearchPage.process(toProcess))
		{
		}
		else
		if(iContactPage.process(toProcess))
		{
		}
		else
		...
		if(iLogout.process(toProcess))
		{
		}
		else
		if(iAddAnimal.process(toProcess))
		{
		}
		else
		if(iAddPlant.process(toProcess))
		{
		}
		else
		if(iLoginSubmit.process(toProcess))
		{
		}
		else
		if(iEditAccountPage.process(toProcess))
		{
		}
		else
		if(iAnimalEdit.process(toProcess))
		{
		}
		else
		if(iPlantEdit.process(toProcess))
		{
		}
		else
